...
| generateVoiceovers | OUT | Admits cost via admit(); appends CostProjected; dispatches one TTSJob per scene (the CaptionJob is NOT dispatched here — reactions.ts dispatches it after all VoiceSynthesized land, so the tts word-timestamp sidecars are guaranteed on disk) | in-process | api/src/commands.ts |
